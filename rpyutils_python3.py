...
def get_frequency(channel):
    if channel == 14:
        ...
        freq = 2407 + (channel * 5)
    """
    I am unable to convert freq to bytes. I have tried different things but none of them has worked properly
    freq_low_byte = freq % 256
    freq_high_byte = freq // 256

    print("freq_low_byte:")
    print(freq_low_byte)
    freq_string = freq_low_byte.to_bytes(1, byteorder='little').hex() 
    #+ freq_low_byte.to_bytes(1, byteorder='little')
    #freq_string = struct.pack("<h", freq)   # Return a bytes object containing the value freq, packed according to the format '<', which means little endian
    print("freq_string:")
    print(freq_string)
    return freq_string
    """
